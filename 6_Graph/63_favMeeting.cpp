...
/*
Topologcal sort can be used find redundant node(acyclic nodes)
*/
class Solution
{
public:
  // https://leetcode.com/problems/maximum-employees-to-be-invited-to-a-meeting/solutions/1660944/c-dfs-with-illustration/
  int maximumInvitations(vector<int> &A)
  {
    ...
    {
      if (m[u] != -1)
        return {u, m[u], false}; // We visited this node the second time, so this node must be the entry point to the cycle
      m[u] = 0;
...
// less runtime but non intiutive most relevant
class Solution
{
public:
  int maximumInvitations(vector<int> &favorite)
  {
    ...
    vector<int> depths(n, 1); // depths[i] is the longest path leading to i exclusively.
    ...
    int maxCircle = 0; // loops of length > 2.
    int maxLink = 0;   // loops of length 2 and paths leading to both nodes.
